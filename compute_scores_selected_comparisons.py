...
Z_exp2_np = np.random.multivariate_normal(mean=np.zeros(101), cov=correlation_exp2, size=n_members)
...
u_exp2_np = norm.cdf(Z_exp2_np)
u_cutoff_np = norm.cdf(Z_cutoff_np)

np.savez(
    cache_dir + "uniform_samples.npz",
    u_exp=u_exp_np,
    u_exp2=u_exp2_np,
    u_cutoff=u_cutoff_np,
)
